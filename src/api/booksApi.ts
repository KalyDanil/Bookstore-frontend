import { api } from '.';
import type { IBook, BooksResponseType, GenresResponseType } from '../types/book';
import type { IAddBookToCartReq, IChangeRatingReq, IComment, IDeleteBookFromCartReq, IGetBooksReq, IGetCartBooksReq, IGetLikedBooksReq, IGetRecommendationsReq, IGetSelectedBookReq, IMakeCommentReq, IToLikeBookReq, CartBooksResponseType } from '../types/userBooks';

export const getBooks = async (params: IGetBooksReq) => {
  const res: { books: BooksResponseType; booksCount: number; minPrice: number; maxPrice: number; genres: GenresResponseType } = await api.get('main/', { params });
  return res;
};
